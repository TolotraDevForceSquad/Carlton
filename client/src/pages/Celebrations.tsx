...
const Celebrations = () => {
  const { heroTitle, heroSubtitle, heroImage, content, event, ctaTitle, ctaDescription, ctaButton } = celebrationsData;

  const processedEventData = {
    ...event,
    image: event.image || eventImage,
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Card className="overflow-hidden hover-elevate transition-all duration-300 flex flex-col lg:flex-row">
            <div className="lg:w-1/2">
              <img 
                src={processedEventData.image} 
                alt={processedEventData.name}
                ...
                <CardHeader className="p-0 mb-6">
                  <div className="flex items-center gap-4 mb-4">
                    <Badge variant="outline" className="text-primary border-primary">
                      ...
                      {processedEventData.capacity}
                    </Badge>
                    <Badge variant="outline" className="text-primary border-primary">
                      <Clock className="w-3 h-3 mr-1" />
                      {processedEventData.duration}
                    </Badge>
                  </div>
                  <CardTitle className="text-3xl font-serif text-foreground mb-2">
                    {formatAmpersand(processedEventData.name)}
                  </CardTitle>
                  <p className="text-primary font-luxury italic text-lg mb-4">
                    {processedEventData.subtitle}
                  </p>
                </CardHeader>
                
                <CardContent className="p-0 space-y-6">
                  <p className="text-muted-foreground leading-relaxed text-lg">
                    {processedEventData.description}
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        {processedEventData.equipment.map((item, idx) => (
                    ...
                    <div>
                      <h4 className="font-semibold text-foreground mb-3">Prestations :</h4>
                      <div className="space-y-2">
                        {processedEventData.features.map((feature, idx) => (
                          <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
                            <Star className="w-3 h-3 text-primary mt-1 flex-shrink-0" />
                            <span>{feature}</span>
                          </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {processedEventData.venues.map((venue, idx) => (
                        <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
      ...
      <section className="py-16 bg-gradient-to-r from-primary/10 to-accent/10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Button size="lg" data-testid="button-contact-celebrations">
            {ctaButton}
          </Button>
        </div>
      </section>
