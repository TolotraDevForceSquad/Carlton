...
import { corporatePageData } from '@/data/corporateData';

const Corporate = () => {
  const pageData = corporatePageData;
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Card className="overflow-hidden hover-elevate transition-all duration-300 flex flex-col lg:flex-row-reverse">
            <div className="lg:w-1/2">
              <img 
                src={corporateData.image} 
                alt={corporateData.name}
            ...
            <div className="lg:w-1/2 p-8 flex flex-col justify-between">
              <div>
                <CardHeader className="p-0 mb-6">
                  <div className="flex items-center gap-4 mb-4">
                    <Badge variant="outline" className="text-primary border-primary">
                      ...
                      {corporateData.capacity}
                    </Badge>
                    <Badge variant="outline" className="text-primary border-primary">
                      <Clock className="w-3 h-3 mr-1" />
                      {corporateData.duration}
                    </Badge>
                  </div>
                  <CardTitle className="text-3xl font-serif text-foreground mb-2">
                    {formatAmpersand(corporateData.name)}
                  </CardTitle>
                ...
                <CardContent className="p-0 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        {(corporateData.equipment || []).map((item, idx) => (
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {(corporateData.features || []).map((feature, idx) => (
                          <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
                            <Star className="w-3 h-3 text-primary mt-1 flex-shrink-0" />
                            <span>{feature}</span>
                          </div>
                  ...
                  <div>
                    <h4 className="font-semibold text-foreground mb-3">Espaces disponibles :</h4>
                    <div className="space-y-2">
                      {corporateData.venues.map((venue, idx) => (
                        <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
      ...
      <section className="py-16 bg-gradient-to-r from-primary/10 to-accent/10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Button size="lg" data-testid="button-contact-corporate">
            {ctaData.buttonText}
          </Button>
        </div>
      </section>
