...
import { galasPageData } from '@/data/galasData';

const Galas = () => {
  const pageData = galasPageData;
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Card className="overflow-hidden hover-elevate transition-all duration-300 flex flex-col lg:flex-row-reverse">
              ...
              <div>
                <CardHeader className="p-0 mb-6">
                  ...
                  <CardTitle className="text-3xl font-serif text-foreground mb-2">
                    {formatAmpersand(galasData.name)}
                  </CardTitle>
                ...
                <CardContent className="p-0 space-y-6">
                  <p className="text-muted-foreground leading-relaxed text-lg">
                    {galasData.description}
                  </p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="font-semibold text-foreground mb-3">Prestations techniques :</h4>
                      <div className="space-y-2">
                        {(galasData.equipment || []).map((item, idx) => (
                    ...
                    <div>
                      <h4 className="font-semibold text-foreground mb-3">Services VIP :</h4>
                      <div className="space-y-2">
                        {(galasData.features || []).map((feature, idx) => (
                          <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
                            <Star className="w-3 h-3 text-primary mt-1 flex-shrink-0" />
                            <span>{feature}</span>
                          </div>
                  ...
                  <div>
                    <h4 className="font-semibold text-foreground mb-3">Espaces prestige :</h4>
                    <div className="space-y-2">
                      {galasData.venues.map((venue, idx) => (
                        <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
      ...
      <section className="py-16 bg-gradient-to-r from-primary/10 to-accent/10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Button size="lg" data-testid="button-contact-galas">
            {ctaData.buttonText}
          </Button>
        </div>
      </section>
