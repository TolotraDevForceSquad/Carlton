...
const Home = () => {
  ...
  const fullParagraphs = fullContent.split('\n\n');
  const initialContent = fullParagraphs[0]; // Premier paragraphe seulement
  const additionalParagraphs = fullParagraphs.slice(1); // Les 4 paragraphes suivants
  const showExpandButton = additionalParagraphs.length > 0;
        ...
        <section className="py-20 bg-card/30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <div className="max-w-5xl mx-auto"> {/* Étiré horizontalement : max-w-5xl au lieu de max-w-4xl */}
                <Tooltip 
                  frLabel={data.content.fr} 
                  enLabel={data.content.en} 
                  onSave={updateContent}
                >
                  {/* Contenu initial (premier paragraphe seulement, sans justification) */}
                  <div className="text-xl text-muted-foreground leading-relaxed prose prose-lg dark:prose-invert mb-8">
                    <p>{initialContent}</p>
                  </div>
                </Tooltip>
                
                {/* Bouton toujours visible après le premier paragraphe */}
                {showExpandButton && (
                ...
                {/* Contenu additionnel révélé au clic : 2x2 grille avec conteneurs gris sombre, justifiés */}
                {isContentExpanded && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {additionalParagraphs.map((para, i) => (
                      <div key={i} className="bg-muted/80 dark:bg-muted p-6 rounded-lg shadow-md"> {/* Conteneur gris sombre */}
